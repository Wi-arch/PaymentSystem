...
public class MySQLRequestParameterDAO extends SQLUtil implements RequestParameterDAO {
	...
	private static final String GET_REQUEST_PARAMETER_BY_ID = "SELECT * FROM request_parameter INNER JOIN parameter_header "
			+ "ON request_parameter.parameter_header_name = parameter_header.parameter_header_name "
			+ "INNER JOIN user_request ON user_request.user_request_id = request_parameter.user_request_id "
			+ "INNER JOIN request_type ON request_type.request_type_value = user_request.request_type_value "
			+ "INNER JOIN request_status ON request_status.request_status_value = user_request.request_status_value "
			+ "INNER JOIN bank_user ON bank_user.bank_user_login = user_request.bank_user_login "
